...
def export_pdf():
    ...
    elements.append(Paragraph(f"Generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | User: {current_user.name}", subtitle_style))
    ...
    for r in readings[-100:]: # Limit to last 100 for PDF readability
        ts = r['timestamp']
    ...
    if len(table_data) > 1:
        ...
        elements.append(Paragraph("No data records found for this period.", styles['Italic']))
